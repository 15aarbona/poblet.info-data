...
class TextRedirector(object):
    """Redirige el stdout a un widget de texto de tkinter."""
...
class PipelineApp(ctk.CTk):
    def __init__(self):
        ...
        self.var_procesar = ctk.BooleanVar(value=True) # Unifica Neteja y Transformació (Astro)
    ...
    def _construir_tab_pipeline(self):
        ...
        ctk.CTkCheckBox(frame_der, text="Netejar i Generar JSONs (Astro)", variable=self.var_procesar).pack(pady=5, padx=20, anchor="w")
    ...
    # ==========================================
    #     LÓGICA DEL PIPELINE (EXTRACCIÓN)
    # ==========================================
    def _reportar_estado_ui(self, red, creador, accion, total=None):
        if accion == "init" and total is not None:
            print(f"📡 Iniciant {red} amb {total} creadors...")
        elif accion == "done" and creador:
            print(f"  ✔️ {creador} completat a {red}")
        ...
        try:
            ...
            if self.var_extraer.get():
                if not redes_activas:
                    print("[!] No has seleccionat cap xarxa per extreure.")
                else:
                    print(f"🚀 Iniciant Extracció per: {', '.join(redes_activas)}\n")
                    asyncio.run(self._ejecutar_extracciones_paralelas(redes_activas))
                
            # FASE 2: Transformación (Cleaner + Transformer)
            if self.var_procesar.get():
                print("\n🧹 [FASE 2] Iniciant Neteja de Dades...")
                DataCleaner().process()
                
                print("\n⚙️ [FASE 3] Transformant a JSONs per a la Web...")
    ...
    async def _ejecutar_extracciones_paralelas(self, redes):
        tareas = []
        clases = {
        ...
        for red in redes:
            if red in clases:
                instancia = clases[red](ui_callback=self._reportar_estado_ui)
                tareas.append(instancia._extraccion())
